...
def generate_list(root, split='train',task ='noobject', download=False):
    from lib.datasets._util import download as download_data, check_exits
    import json
    ...
    root = os.path.join(root, "H3D_crop")
    # load labels
...
def data_split_mse(args, model, tgt_list=None, NUM=None): 
    ...
    # print(outbank.size(), entbank.size(), labelbank.size())
    acc_per_points, avg_acc, cnt, pred1 = accuracy(outbank.cpu().numpy(),
                                                    labelbank.cpu().numpy())
    # _, _, _, pred2 = accuracy(labelbank.cpu().numpy(),labelbank.cpu().numpy())
    ###############################################################################
    pred_list = []

    for i in tqdm(range(len(tgt_list))):
        item = tgt_list[i]
        kp = pred1[i] * args.image_size / args.heatmap_size
        kp[:,0] = kp[:,0] * orig_shape / args.image_size
        kp[:,1] = kp[:,1] * orig_shape / args.image_size
        item_tmp = {'name':item['name'], 'keypoint2d':kp.tolist(), 'keypoint3d':item['keypoint3d'],'intrinsic_matrix':item['intrinsic_matrix'], 'without_object':item['without_object']}
        # item['keypoint2d'] = kp.tolist()
        # print(item)
        # exit(0)
        pred_list.append(item_tmp)
    # target_dataset = datasets.__dict__[args.target]
    # val_pred_dataset = target_dataset(root=args.target_root, tgt_list=pred_list, split='train', transforms=val_transform,
    #                                     image_size=image_size, heatmap_size=heatmap_size)
    ...
    val_pred_dataset = proxy_dataset(root=args.target_root, tgt_list = pred_list, transforms_base=base_transform,
                                        transforms_stu=tgt_train_transform_stu, transforms_tea=tgt_train_transform_tea, label_transform = label_transform,
    ...
    if NUM is None:
        NUM = int(msebank.size(0) * pct)
    sort_idx = torch.argsort(msebank)
    print('Mean mse:', msebank.mean())
    ...
    # print('MeanEnt:', entbank.mean())

    # exit(0)
    easy_idx = sort_idx[:NUM]
    # hard_idx = sort_idx[NUM:]
    ...
    for i in range(NUM):
        ...
        # if args.test:
        #     kp = pred2[ind] * args.image_size / args.heatmap_size
        # else:
        kp = pred1[ind] * args.image_size / args.heatmap_size
        kp[:,0] = kp[:,0] * orig_shape / args.image_size
        kp[:,1] = kp[:,1] * orig_shape / args.image_size
        # print(kp.tolist())
        # print(item['keypoint2d'] , kp.tolist())
        item['keypoint2d'] = kp.tolist()
        # print(item)
        # exit(0)
...
def main(args: argparse.Namespace):
    ...
    student = models.__dict__[args.arch](num_keypoints=21).cuda()
    teacher = models.__dict__[args.arch](num_keypoints=21).cuda()
    source_model = models.__dict__[args.arch](num_keypoints=21).cuda()
    ...
    def visualize(image, keypoint2d, name):
        ...
        """
        train_source_dataset.visualize(tensor_to_image(image), keypoint2d, logger.get_image_path("{}.jpg".format(name)))
    if args.vsl:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train(train_proxy_iter, train_target_iter, source_model, student, teacher,  criterion, con_criterion, 
...
def train(train_source_iter, train_target_iter, source_model, student, teacher,  criterion, con_criterion,
    ...
    for i in range(args.iters_per_epoch):
        ...
        x_s, label_s, _, meta_s = next(train_source_iter)
        # x_s, label_s, w, meta_s, _, _, _, _,  img_lb= next(train_source_iter)

        weight_s = torch.ones((x_s.size(0), 21,1))
        # print(weight_s)
...
if __name__ == '__main__':
    ...
    # dataset parameters
    parser.add_argument('--source_root', default='hand_data/RHD', help='root path of the source dataset')
    parser.add_argument('--target_root', default='hand_data/H3D_crop', help='root path of the target dataset')
    ...
    parser.add_argument('--target-train', default='Hand3DStudio_mt', help='target domain(s)')
    ...
    parser.add_argument('--lr-step', default=[10, 20], type=tuple, help='parameter for lr scheduler')
    parser.add_argument('--lr-factor', default=1, type=float, help='parameter for lr scheduler')
    ...
    parser.add_argument("--log", type=str, default='target',
    ...
    parser.add_argument('--source_ckpt', default='./logs/hand/source/final.pt', type=str)
    

    args = parser.parse_args()
    args.log = 'logs/H3D/' + args.log
    main(args)
