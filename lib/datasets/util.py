...
def crop_ori(img, center, scale, res=(256 ,256), rot=0):
    img = im_to_numpy(img)
    # print(center, scale, img.size)

    # Preprocessing for efficient cropping
    ht, wd = img.shape[0], img.shape[1] # 225, 400
    ...
    if sf < 2:
        ...
        # print(new_size, new_ht, new_wd, ht, wd, sf)
        if new_size < 2:
    ...
    # raise RuntimeError
    return new_img
def crop_ori_256(img, center, scale, res=(256 ,256), rot=0):
    # if img.size()[1] == 256:
    #     tag=1
    #     img = im_to_numpy(img)
    #     return img
