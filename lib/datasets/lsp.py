...
class LSP(Body16KeypointDataset):
    ...
    """
    def __init__(self, root, tgt_list=None, split='train', task='all', download=True, image_size=(256, 256), transforms=None, kp_reverse=False, **kwargs):
        if download:
    ...
    def __getitem__(self, index):
        ...
        # if index==581:
        #     print(sample)
        #     print('orig',keypoint2d)
        image, data = self.transforms(image, keypoint2d=keypoint2d, kp_reverse=self.kp_reverse)
        # if index==581:
        #     print('trans',data['keypoint2d'])
        if self.kp_reverse:
            return data['keypoint2d'], index
